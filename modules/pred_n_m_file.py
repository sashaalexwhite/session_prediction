...
model = load_model()
encoder = joblib.load(f'{path}/data/modele/encoder.pkl')
scaler = joblib.load(f'{path}/data/modele/scaler.pkl')
...
def pred_n_m_file():
    folder_path = f'{path}/data/n_m_file'
    ...
    for file_name in os.listdir(folder_path):
        if file_name.endswith('.parquet'):
            ...
            X, session_client_data = process_data(file_path, encoder, scaler)  # передача scaler
            probabilities = model.predict_proba(X)[:, 1]
            predictions = model.predict(X)

            # Добавляем колонки с вероятностями и классификацией
            data = pd.read_parquet(file_path)  # Загрузим файл для добавления колонки
    ...
    # Очистка локальной папки после обработки всех файлов
    for file_name in os.listdir(folder_path):
        file_path = os.path.join(folder_path, file_name)
        os.remove(file_path)
